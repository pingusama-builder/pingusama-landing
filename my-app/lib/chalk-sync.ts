...
export function validateSnapshot(input: unknown): Snapshot {
    ...
    if (x.schema !== 1 || x.year !== 2026 || typeof x.published !== 'boolean')
        fail();
    ...
    const habits = list(x.habits, 2).map(raw => { const h = obj(raw); keys(h, ['key', 'slot', 'name', 'since']); const key = text(h.key, 64), slot = int(h.slot, 1, 2), name = text(h.name, 40); if (!/^[a-zA-Z0-9_-]+$/.test(key) || name.trim() !== name || usedSlots.has(slot) || usedKeys.has(key))
        fail(); usedSlots.add(slot); usedKeys.add(key); return { key, slot, name, since: date(h.since) }; });
    const seen = new Set<string>();
    let points = 0;
    const rows = list(x.rows, 730).map(raw => {
        const r = obj(raw);
        keys(r, ['device_id', 'habit_key', 'habit_slot', 'day', 'marked_at', 'strokes']);
        ...
        if (seen.has(id))
            fail();
        seen.add(id);
        const marked_at = text(r.marked_at, 40);
        if (!/^\d{4}-\d\d-\d\dT/.test(marked_at) || !Number.isFinite(Date.parse(marked_at)))
            fail();
        ...
        if (!strokes.length)
            fail();
        return { device_id: device, habit_key: h.key, habit_slot: h.slot, day, marked_at, strokes };
    });
    if (!x.published && (habits.length || rows.length))
        fail();
    return { schema: 1, device_id: device, revision, published: x.published, year: 2026, habits, rows };
}
export function publicCalendar(row: StoredCalendar | null) { return { published: row?.published === true, year: 2026, publishedAt: row?.accepted_at ?? null, habits: row?.published ? row.payload.habits : [], rows: row?.published ? row.payload.rows.map(r => ({ habit_key: r.habit_key, habit_slot: r.habit_slot, day: r.day, strokes: r.strokes })) : [] }; }
