...
### Atlas / 「現在・工坊」
- **State:** visual direction locked and deployed; Atlas v2 workshop foundation + Pingusama invitation motion blocking now integrated into `my-app/public/portrait/atlas.html` as a full-viewport v2 scene and live at `https://pingu-tinkering.vercel.app/portrait/atlas` (deployment `dpl_AGio1ARUoLK884TgkJdJV9UCNVRQ`). Peer-room pages `/portrait/vn` and `/portrait/books` are also deployed (`dpl_J4RTESuq5fWThpPx4rTyzMyQYYUD`); `world-name-of-the-wind` is abandoned and removed. Previous canvas-based night-ink atlas preserved as `my-app/public/portrait/atlas-night-ink-v2.html`. Preserve both foundation image time layers: `visuals/portrait-atlas-foundation-2026-07-26.png` and `visuals/portrait-atlas-foundation-v2-workshop-2026-07-26.png`.
...
- **Substrate experiments:** scroll-world / parallax was probed 2026-07-27 with two throwaway prototypes (`atlas-scroll-world-prototype.html` horizontal drag; `atlas-scroll-world-light.html` vertical scroll-scrub with section copy). Verdict: effect not strong enough; direction abandoned. A 2.5D depth shader probe (`atlas-img2threejs-probe.html`) was also built 2026-07-28 as a throwaway browser-native alternative to the full img2threejs agentic pipeline.
- **Entrance / site topology:** DEPLOYED. Portrait removed from top nav (`Header.tsx`); added as a `portrait-tile` link inside the hero scene in `LandingPage.tsx`, beside the bench wagon, tool wheel, and runner. The tile carries a built-in SVG ink-wash thumbnail in the 天之痕 palette (aged paper, bronze frame, jade mist, pine-ink mountains, cinnabar seal, bronze corner studs). Responsive: desktop = vertical tile with label/sub; mobile ≤640px = horizontal row above the wheel with compact thumbnail and hidden subtitle. Hover/focus-visible lift + shadow. Deployed via isolated branch `deploy/portrait-entrance-v2` off the Atlas-fix base; deployment ID `dpl_BF36xJNe6MAN1Tyc4NAZWmq5TChx` @ https://pingu-tinkering.vercel.app. Verified build + tests + anon smoke.
...
- **Book-world 3D prototype — The Eolian DONE 2026-07-23 (not deployed).** First single-book 3D world: `my-app/public/portrait/world-name-of-the-wind.html` — self-contained Three.js atmospheric music-hall scene for *The Name of the Wind*. Procedural geometry (stage, floating lute, tables/chairs, bar, pillars), warm spotlight, dust particles, orbit camera, raycast click interaction showing four excerpts from Robin's note. No external assets. Route `/portrait/world-name-of-the-wind` wired in `next.config.ts`. Build + tests green; committed `83f9cf7`; deploy pending explicit go-ahead.
